...
<div class="checkout-summary flex flex-col gap-3">
    ...
    <ul class="checkout-summary__rules list-none m-0 p-0 flex flex-col gap-1.5">
        @foreach($cartPricing['applied_rules'] as $rule)
        <li class="flex items-center justify-between gap-2 text-sm text-body py-1 border-b border-border-subtle/50 last:border-0">
            <span class="inline-flex items-center gap-1.5 font-medium text-headings">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-success shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                    <polyline points="20 6 9 17 4 12" />
                </svg>
                {{ $rule['rule_name'] ?? t('Discount') }}
            </span>
            @if(!empty($rule['free_shipping']))
            <span class="inline-flex items-center px-2 py-0.5 rounded bg-success-subtle text-success-subtle-content text-xs font-semibold border border-success/20">@t('Free shipping')</span>
            @elseif(!empty($rule['discount']))
            <span class="inline-flex items-center px-2 py-0.5 rounded bg-success-subtle text-success-subtle-content text-xs font-semibold border border-success/20">-{{ formatCurrency($rule['discount']) }}</span>
            @endif
        </li>
        @endforeach
    </ul>
