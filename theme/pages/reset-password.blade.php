{{--
    Reset password page — split-screen layout (mirrors login.blade.php).

    Reached via `/{locale}/reset-password?token=…&email=…`. If either query
    param is missing we don't render the form at all — the link is malformed or
    expired. POSTs to `store.reset-password.post` via `@storefrontForm`.
    Passwords are NEVER re-populated from `old()`. Every color / font / spacing
    value resolves through a design token (docs/styling.md, docs/tokens.md).
--}}
        ...
        <section class="auth__panel flex flex-1 flex-col overflow-y-auto bg-surface-page p-6 lg:p-10">
            <div class="auth__form-wrap m-auto w-full max-w-420">
                ...
                @if(empty($token) || empty($email))
                    <div class="auth__flash mt-6 rounded-lg border border-surface-error-subtle-stroke bg-surface-error-subtle px-3 py-2 text-sm text-surface-error-subtle-content">
                        @t('This reset link is invalid or has expired. Please request a new one.')
                    </div>
                    <a href="@routeUrl('store.forgot-password')" class="auth__back mt-6 inline-flex items-center justify-center gap-1.5 text-sm font-medium text-primary hover:text-primary-600 hover:no-underline">
                        @t('Request a new reset code')
                    </a>
                @else
                    @storefrontForm('reset-password', ['class' => 'auth__form mt-6 flex flex-col gap-4'])
                        <input type="hidden" name="token" value="{{ $token }}">
                        <input type="hidden" name="email" value="{{ $email }}">

                        {{-- New password --}}
                        <div class="auth__field flex flex-col gap-1.5">
                            <label for="reset-password" class="auth__label text-sm font-medium text-body">@t('New password')</label>
                            ...
                            @if(!empty($passwordRules))
                                <ul class="auth__password-hints mt-2 flex flex-col gap-1.5" data-password-checklist="reset-password">
                        ...
                        {{-- Confirm password --}}
                        <div class="auth__field flex flex-col gap-1.5">
                            <label for="reset-password-confirm" class="auth__label text-sm font-medium text-body">@t('Confirm password')</label>
                            <div class="auth__input flex items-center gap-2 rounded-lg border border-surface-input-stroke bg-surface-input px-3 py-2 focus-within:border-primary">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="auth__input-icon h-5 w-5 shrink-0 text-placeholder" aria-hidden="true">
                                    <rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect>
                                    <path d="M7 11V7a5 5 0 0 1 10 0v4"></path>
