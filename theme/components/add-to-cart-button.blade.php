...
<a
    ...
    @class([ 'add-to-cart add-to-cart--options inline-flex items-center justify-center gap-2 rounded bg-primary text-primary-content font-medium px-4 py-2 transition-colors hover:bg-primary-600 hover:no-underline' , 'w-full'=> $block,
    'min-w-40' => !$block,
    ])
    >
    @if($icon)
...
<button
    type="submit"
    @class([ 'add-to-cart__button inline-flex flex-1 items-center justify-center gap-2 rounded bg-primary text-primary-content font-medium px-3 py-2 transition-colors hover:bg-primary-600 active:bg-primary-700 cursor-pointer' , 'w-full'=> $block && !($showQuantity ?? true),
    'min-w-32' => !$block && !($showQuantity ?? true),
    ])
    >
    @if($icon)
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4" aria-hidden="true">
